...
def main(args):
    ...
    for i in range(n_modules):
        ...
        geneExprDf_blocks.append(temp)
        
    geneExprDf = pd.concat(geneExprDf_blocks, ignore_index=True)
    ...
    df = geneExprDf.apply(pd.to_numeric, errors='coerce').fillna(0)
    df.index = [int(str(i).split('_')[0]) for i in df.index]
